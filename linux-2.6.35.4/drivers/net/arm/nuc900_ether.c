...
struct recv_pdesc {
	struct nuc900_rxbd desclist[RX_DESC_SIZE];
...
static void update_linkspeed(struct net_device *dev)
{
	...
	bmsr = nuc900_mdio_read(dev, ether->mii.phy_id, MII_BMSR);
	bmcr = nuc900_mdio_read(dev, ether->mii.phy_id, MII_BMCR);

	if (bmcr & BMCR_ANENABLE) {
		if (!(bmsr & BMSR_ANEGCOMPLETE))
			return;

		lpa = nuc900_mdio_read(dev, ether->mii.phy_id, MII_LPA);
...
static void nuc900_check_link(unsigned long dev_id)
{
...
static int nuc900_init_desc(struct net_device *dev)
{
	...
	struct nuc900_txbd  *tdesc;
	struct nuc900_rxbd  *rdesc;
	struct platform_device *pdev;
	unsigned int i;
...
static void nuc900_reset_mac(struct net_device *dev)
{
	...
	nuc900_set_curdest(dev);
	nuc900_enable_cam(dev);
	nuc900_enable_cam_command(dev);
	nuc900_enable_mac_interrupt(dev);
	nuc900_enable_tx(dev, 1);
	nuc900_enable_rx(dev, 1);
	nuc900_trigger_tx(dev);
...
static void nuc900_mdio_write(struct net_device *dev,
...
static int nuc900_set_mac_address(struct net_device *dev, void *addr)
{
...
static int nuc900_ether_close(struct net_device *dev)
{
	struct nuc900_ether *ether = netdev_priv(dev);
	struct platform_device *pdev;
...
static struct net_device_stats *nuc900_ether_stats(struct net_device *dev)
{
	struct nuc900_ether *ether;
...
static int nuc900_send_frame(struct net_device *dev,
...
static irqreturn_t nuc900_tx_interrupt(int irq, void *dev_id)
{
	struct nuc900_ether *ether;
	struct nuc900_txbd  *txbd;
	struct platform_device *pdev;
...
static int nuc900_ether_open(struct net_device *dev)
{
	struct nuc900_ether *ether;
	struct platform_device *pdev;
	...
	nuc900_enable_cam_command(dev);
	nuc900_enable_mac_interrupt(dev);
	nuc900_set_global_maccmd(dev);
	nuc900_enable_rx(dev, 1);

	ether->rx_packets = 0x0;
	ether->rx_bytes = 0x0;

	if (request_irq(ether->txirq, nuc900_tx_interrupt,
						0x0, pdev->name, dev)) {
		dev_err(&pdev->dev, "register irq tx failed\n");
		return -EAGAIN;
	}

	if (request_irq(ether->rxirq, nuc900_rx_interrupt,
						0x0, pdev->name, dev)) {
...
static void nuc900_ether_set_multicast_list(struct net_device *dev)
{
	struct nuc900_ether *ether;
...
static void nuc900_get_drvinfo(struct net_device *dev,
...
static int nuc900_get_settings(struct net_device *dev, struct ethtool_cmd *cmd)
{
	struct nuc900_ether *ether = netdev_priv(dev);
	return mii_ethtool_gset(&ether->mii, cmd);
}

static int nuc900_set_settings(struct net_device *dev, struct ethtool_cmd *cmd)
{
	struct nuc900_ether *ether = netdev_priv(dev);
	return mii_ethtool_sset(&ether->mii, cmd);
}

static int nuc900_nway_reset(struct net_device *dev)
{
	struct nuc900_ether *ether = netdev_priv(dev);
	return mii_nway_restart(&ether->mii);
}

static u32 nuc900_get_link(struct net_device *dev)
{
	struct nuc900_ether *ether = netdev_priv(dev);
	return mii_link_ok(&ether->mii);
}

static const struct ethtool_ops nuc900_ether_ethtool_ops = {
	.get_settings	= nuc900_get_settings,
	.set_settings	= nuc900_set_settings,
	.get_drvinfo	= nuc900_get_drvinfo,
	.nway_reset	= nuc900_nway_reset,
	.get_link	= nuc900_get_link,
};

static const struct net_device_ops nuc900_ether_netdev_ops = {
	.ndo_open		= nuc900_ether_open,
	.ndo_stop		= nuc900_ether_close,
	.ndo_start_xmit		= nuc900_ether_start_xmit,
	.ndo_get_stats		= nuc900_ether_stats,
	.ndo_set_multicast_list	= nuc900_ether_set_multicast_list,
	.ndo_set_mac_address	= nuc900_set_mac_address,
	.ndo_do_ioctl		= nuc900_ether_ioctl,
...
static void __init get_mac_address(struct net_device *dev)
{
	...
	if (is_valid_ether_addr(nuc900_mac))
		memcpy(dev->dev_addr, &nuc900_mac[0], 0x06);
	else
		dev_err(&pdev->dev, "invalid mac address\n");
}

static int nuc900_ether_setup(struct net_device *dev)
{
	struct nuc900_ether *ether = netdev_priv(dev);

	ether_setup(dev);
	dev->netdev_ops = &nuc900_ether_netdev_ops;
	dev->ethtool_ops = &nuc900_ether_ethtool_ops;
	...
	ether->mii.dev = dev;
	ether->mii.mdio_read = nuc900_mdio_read;
	ether->mii.mdio_write = nuc900_mdio_write;

	setup_timer(&ether->check_timer, nuc900_check_link,
						(unsigned long)dev);

	return 0;
}

static int __devinit nuc900_ether_probe(struct platform_device *pdev)
{
	struct nuc900_ether *ether;
	struct net_device *dev;
	int error;

	dev = alloc_etherdev(sizeof(struct nuc900_ether));
	if (!dev)
		return -ENOMEM;
	...
	nuc900_ether_setup(dev);

	error = register_netdev(dev);
	if (error != 0) {
		dev_err(&pdev->dev, "Regiter EMC nuc900 FAILED\n");
		error = -ENODEV;
...
static int __devexit nuc900_ether_remove(struct platform_device *pdev)
{
...
static struct platform_driver nuc900_ether_driver = {
	.probe		= nuc900_ether_probe,
	.remove		= __devexit_p(nuc900_ether_remove),
	.driver		= {
		.name	= "nuc900-emc",
		.owner	= THIS_MODULE,
	},
...
static void __exit nuc900_ether_exit(void)
{
	platform_driver_unregister(&nuc900_ether_driver);
}

module_init(nuc900_ether_init);
module_exit(nuc900_ether_exit);

MODULE_AUTHOR("Wan ZongShun <mcuos.com@gmail.com>");
MODULE_DESCRIPTION("nuc900 MAC driver!");
MODULE_LICENSE("GPL");
